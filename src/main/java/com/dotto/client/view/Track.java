...
public class Track implements View {
    ...
    private void setDiscordTime() {
        Discord.discord.endTimestamp = (System.currentTimeMillis() + (music.clip.getMicrosecondLength() / 1000)) / 1000;
        Discord.update();
    }
    ...
    public void draw() {
        // draw back
        back.bind();
        GraphKit.drawQuad(0, 0, Config.WIDTH, Config.HEIGHT);
        ...
        for (int y = -100; y < Config.HEIGHT + 100; y += 100) {
            for (int x = -100; x < Config.WIDTH + 100; x += 100) {
                xpos = x + (xOffset % 200.0f) * 0.5f - Skin.gridPoint.getWidth() / 2;
                ypos = y + (yOffset % 200.0f) * 0.5f - Skin.gridPoint.getHeight() / 2;
                GraphKit.drawQuad(xpos, ypos, Skin.gridPoint.getWidth(), Skin.gridPoint.getHeight());
            }
        }
    ...
    public void update(float delta) {
        ...
        if (xOffset > 0)
            xOffset = 0;
        if (xOffset < -map.bound.x)
            xOffset = -map.bound.x;
        if (yOffset > 0)
            yOffset = 0;
        if (yOffset < -map.bound.y)
            yOffset = -map.bound.y;
